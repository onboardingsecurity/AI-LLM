...
> 이후 변경(사용자 요청): 문구 크기 조작 범위를 16~100으로 줄였고, 이미지를 넣기 전에는 문구를 입력할 수 없게 했다("이미지 없이 시작"을 누르면 흰 카드로 열린다). 아래 기록은 그 전에 돌린 결과다. 특히 E11의 크기 240은 지금 화면에서 조작할 수 없다(저장 데이터로는 읽을 수 있다). 지금 화면에서 다시 확인하려면 크기 100으로 바꿔서 돌린다. 또한 문구 입력은 50자까지로 제한했다(요청). "긴 한글" 등 50자를 넘는 입력은 지금 화면에서 직접 입력할 수 없고, 예전에 저장한 템플릿으로만 불러올 수 있다.
